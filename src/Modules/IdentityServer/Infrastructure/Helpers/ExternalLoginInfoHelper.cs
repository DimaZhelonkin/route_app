...
namespace Ark.IdentityServer.Infrastructure.Helpers;
...
public class ExternalLoginInfoHelper
{
    public static (string name, string surname) GetNameAndSurnameFromClaims(List<Claim> claims)
    {
        ...
        if (givennameClaim != null && !string.IsNullOrEmpty(givennameClaim.Value)) name = givennameClaim.Value;

        var surnameClaim = claims.FirstOrDefault(c => c.Type == ClaimTypes.Surname);
        if (surnameClaim != null && !string.IsNullOrEmpty(surnameClaim.Value)) surname = surnameClaim.Value;
