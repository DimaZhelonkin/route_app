...
public class StartRideCommandHandler : CommandHandler<StartRideCommand>
{
    private readonly IDriverRidesRepository _driverRidesRepository;

    public StartRideCommandHandler(IDriverRidesRepository driverRidesRepository)
    {
        _driverRidesRepository = driverRidesRepository;
    }

    public override async Task<Result> Handle(StartRideCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (driverRide is null)
            return Result.Error("StartRideCommand", "Driver ride not found");
        driverRide.Start();
